	...
	class Loginm extends CI_Model{
		...
		function consultar_usuario($user, $pass){
			...
			foreach ($query as $key => $datosusuario) {
				...
				$_SESSION['cus_nombre']= $datosusuario->cus_nombres;
				...
				foreach ($query2 as $key => $datosrol) {
					$_SESSION['nombrerol'] = $datosrol->rol_nombre;
				}

				//Verificamos lenguaje
				$sql = "SELECT lng_abrev FROM crm_lng_lenguaje WHERE lng_com_id=$_SESSION[compania_id]";
				$this->db->trans_start();
				...
				foreach ($query3 as $key => $datosidioma) {
					$_SESSION['lng'] = $datosidioma->lng_abrev;
